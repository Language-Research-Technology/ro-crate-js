...
class Validator {
  ...
  isCrate() {
    ...
    try {
      ...
      this.log({
        id: "validCrateMustBeJson",
        status: "success",
        message: "A JSON-LD document that describes the RO-Crate with structured data in the form of RO-Crate JSON-LD.",
      })
    } catch (error) {
      this.log({
        id: "validCrateCannotParseJson",
        status: "error",
        message: 'this library can not parse this JSON: ' + String(error),
      });
    }
  }
  ...
  checkFiles(fileReferences, crate) {
    ...
    // I have to make it an array in case you didnt instanciate the ROCrate with array true
    let rootParts = [];
    if (Array.isArray(this.crate.rootDataset.hasPart)) {
      rootParts = this.crate.rootDataset.hasPart;
    } else {
      rootParts = { ...this.crate.rootDataset.hasPart };
    }
    for (let part of rootParts) {
      if (!this.crate.getEntity(part['@id'])) {
        fileReferences[part["@id"]] = {
          exists: false,
          inCrate: true
        }
      }
    ...
    function validFileId(id) {
      return id && id.replace(/^\.\//, '') && !id.match(/(^\.{1,2}$)|(^#.*)/) && !isValidUrl(id);
    }
    // First check 
    for (let entity of this.crate.entities()) {
      const entityId = entity["@id"];
      if (entity["@type"].some(t => t in { File: 1, Dataset: 1 }) &&
        validFileId(entityId) && !fileReferences?.[entity["@id"]]) {

        fileReferences[entityId] = {
          exists: false,
          inCrate: true
        };
        this.results.push({
          id: "validCrateDataEntityPath",
          status: "error",
          entity: entity["@id"],
          message: `Data Entity refers to a local file path or directory that does not exist`
        });
      }
    }
    ...
    for (let file of Object.keys(fileReferences)) {
      ...
      if (!this.crate.getEntity(file)) {
        this.results.push({
          id: "validCrateDataEntityInCrateDir",
          status: "warning",
          entity: file,
    ...
    */
    ...
    if (root?.datePublished) {
      for (let dP of root.datePublished) {
        if (dP && (typeof dP !== 'string' || !dP.match(/^\d{4}-?([0]\d|1[0-2])?-?([0-2]\d|3[01])?/))) {
          this.log(
            {
              id,
              status: "error",
