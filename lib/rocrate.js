...
class ROCrate {
     ...
     * @return {boolean}
     * @todo Add test for this addItem without ids and types, ...exceptions?
     */
    addItem(item) {
        if (!item['@id']) {
        ...
        if (!item['@type']) {
            throw new Error('Item does not have a type');
        }
    ...
    /**
     * pushValue
     * Adds a value to an item's property array
     * @param item
     * @param prop
     * @param val
    ...
    getJson() {
        ...
        json["@graph"] = this._serializeGraph();
        return json;
    }
    ...
    /**
     * referenceToItem
     * Find out if a value (could be a string or an object) is a reference to something
    ...
    _toGraph() {
        ...
        this.json_ld["@graph"] = []; // Save space
        this._rootNode = this.getItem(this._rootId);
        if (!this._rootNode) {
    ...
    /**
     * getGraph
     * @returns a linked array of all items
     */
    getGraph() {
        if (this.__graphIndex) {
