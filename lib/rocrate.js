...
class ROCrate {
  ...
  resolveTerm(term) {
    ...
    let contextIndex = this.__contextTermIndex;
    let val = contextIndex.get(term);
    var parts;
    if (val) {
      ...
      if ((parts = val.match(compactRegex))) {
        ...
        const prefixIri = contextIndex.get(parts[1]);
      ...
      val = contextIndex.get(parts[1]) + parts[2];
    }
    return val;
  }
