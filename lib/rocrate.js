...
class ROCrate {
    ...
    constructor(json = {}, config = {}) {
        ...
        this.config = config;
        let defaultMetadataIds = new Set(defaults.roCrateMetadataIDs);
        // init graph
        ...
        for (let i = 0; i < g.length; ++i) {
            let e = g[i];
            e['@id'] = e['@id'] || `#${i}`;
            if (this.#createNode(e, true)) {
                if (defaultMetadataIds.has(e['@id']) && utils.hasType(e, "CreativeWork")) {
                    this.#rootId = e.about["@id"] || e.about[0]["@id"];
                    this.metadataFileEntity = e;
                }
            }
        }
        // validate graph
     ...
     * @param {boolean} allowEmptyEntity
     * @param {boolean} replaceExisting
     * @returns {Node} a newly created or existing node that matches the id
     */
    #createNode(data, allowEmptyEntity = false, replaceExisting = this.config.replaceExisting) {
        let id = data['@id'];
        if (!id) return;
        ...
        if (allowEmptyEntity || Object.keys(data).length > 1) {
            if (n.isValid) {
     ...
     * @param {Node} node
     * @param {string[]} [propNames_] Specify properties to be included. If not specified, links will be created for all properties
     */
    #addReverse(node, propNames_) {
        //if (!this.config.resolveLinks) return;
        let data = node.data;
        let propNames = propNames_ || Object.keys(data);
        for (let prop of propNames) {
            if (!prop.startsWith('@')) {
                let newVals = mapValue(data[prop], val => {
                    let childId = val['@id'];
                    if (childId) {
                        let child = this.#nodeById.get(childId);
                        if (!child) child = this.#createNode(val);
                        let rev = child.base['@reverse'];
                        let revprop = rev[prop] = rev[prop] || [];
                        if (!revprop.includes(data)) revprop.push(node.base);
                        return child.base;
                    }
                    return val;
                });
                if (newVals != null) data[prop] = newVals;
            }
        }
    }

    #removeReverse(node, propNames_) {
        //if (!this.config.resolveLinks) return;
        let data = node.data;
        let propNames = propNames_ || Object.keys(data);
        for (let prop of propNames) {
            if (!prop.startsWith('@')) {
                let newVals = mapValue(data[prop], val => {
                    ...
                    if (childId) {
                        let child = this.#nodeById.get(childId);
                        if (child) {
                            let rev = child.data['@reverse'];
                            let i = rev[prop].indexOf(node.base);
    ...
    addEntity(data, replaceExisting = this.config.replaceExisting) {
        let n = this.#createNode(data, true, replaceExisting);
        return n != null;
    }
     ...
     */
    updateEntity(data) {
        let n = this.#createNode(data, false, true);
        return n != null;
    }
     ...
     * @param {*} newId 
     */
    updateEntityId(currentId, newId) {
     ...
     * @param {*|*[]} values - A value or an array of values. If value is an array, wrap it first in another array.
     */
     ...
     * If resolveLinks is true, any link to other node will be made into nested object.
     * @param {boolean} raw 
     * @return {Array}
     */
    getGraph(raw = false) {
     ...
     */
    addProfile(uri) {
        this.addValues("ro-crate-metadata.json", "conformsTo", { "@id": uri });
    }
     ...
     */
     addIdentifier(options) {
        if (options.identifier && options.name && this.rootDataset) {
            ...
            if (this.addValues(this.rootDataset, 'identifier', entity, false)) {
                return entityId;
            }
        }
    }
    ...
    getIdentifier(name) {
        ...
        /** @type {{'@id':string, '@type':string, value:string, name:string}[]} */
     ...
     * TODO: find circular reference 
     * @param {*} entity 
     * @param {number} depth 
     * @returns 
     */
    getNormalizedTree(entity, depth = 1) {
        let id = (typeof entity === 'string') ? entity : entity['@id'];
        // ensure entity is a valid one
        let root = this.#getRawEntity(id);
                ...
                if (prop === "@type") {
                    ...
                    node[prop] = utils.asArray(val).
                        ...
                        map(v => {
                            ...
                            if (v['@id'] && level < depth) {
                                v = utils.clone(this.#getRawEntity(v['@id']));
    ...
    objectify() {
        ...
        const root = _.clone(this.rootDataset.$$toJSON());
        this.nest(root, {});
    ...
    nest(item, alreadySeen) {
        if (!alreadySeen[item["@id"]]) {
            ...
            for (let prop of Object.keys(item)) {
                var newValues = [];
                for (let val of utils.asArray(item[prop])) {
                    if (val["@id"]) {
