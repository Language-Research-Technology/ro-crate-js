...
class ROCrate {
    ...
    #createNode(id) {
        ...
        if (!n) {
            n = new Node(id || this.uniqueId('entity-')); // give default id
    ...
    #updateNode(n, data, replaceExisting = this.config.replaceExisting) {
        if (n.isValid) {
            if (!replaceExisting || (n.entity === data && n.data === data.$$raw)) return false;
    ...
    #getRawEntity(idOrEntity) {
        ...
        } else { //if (idOrEntity.$$owner === this) {
            return idOrEntity.$$raw ?? idOrEntity;
        }
    }
     ...
     * @return {object} A wrapper for entity that resolves properties as linked objects
     */
    getEntity(id) {
        let n = this.#nodeById.get(id);
        if (n != null && n.isValid) {
     ...
     * @param {*[]|boolean} subgraph - If present and true, all intervening items during
