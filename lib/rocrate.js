...
class ROCrate {
   ...
   * @param {boolean} [config.allowDuplicates] - Allow duplicate values in a property that has multiple values
   */
  constructor(json = {}, config = {}) {
    ...
    // init graph
    this.#context = structuredClone(json["@context"] || defaults.context);
    let g = json["@graph"];
   ...
   */
  #updateNode(node, data, replaceExisting = this.config.replaceExisting, mergeProperties = this.config.mergeProperties) {
    if (node[$size] > 1) {
      if (!replaceExisting || node[$proxy]?.proxy === data) return false;
      if (!mergeProperties) {
        // remove existing data first
        mapProp(node, (prop, v) => v['@id'] && removeReverse(node[$noderef], prop, v));
        node[$proxy]?.revoke();
        node = new Node(node[$noderef]);
        node[$size] = 1;
      }
    } else {
      node[$size] = 1;
  ...
  #addValues(ref, prop, oldValues_, values, allowDuplicates = this.config.allowDuplicates) {
    let oldValues = utils.asArray(oldValues_);
    mapValue(values, v => {
      if (allowDuplicates || !utils.exists(oldValues, v)) {
        let nv = v;
        if (typeof v === 'object') {
  ...
  updateEntity(data, mergeProperties = this.config.mergeProperties) {
    ...
    if (n && n[$size]) return this.#updateNode(n, data, true, mergeProperties);
    return false;
  }
   ...
   */
  addValues(idOrEntity, prop, values, allowDuplicates = this.config.allowDuplicates) {
    let entity = this.#toNode(idOrEntity);
