...
class ROCrate {
   ...
   */
  #updateNode(node, data, { replace = this.config.replace, merge = this.config.merge, recurse, add }) {
   ...
   */
  get context() {
    return Utils.asArray(this.#context);
  }
   ...
   */
  addContext(context) {
    this.#context = Utils.asArrayRef(this.#context);
  ...
  addEntity(data, {replace, recurse} = {}) {
    ...
    let r = this.#updateNode(n, data, { replace, recurse, add: true });
  ...
  addValues(idOrEntity, prop, values, { duplicate } = {}) {
    ...
    if (prop in entity) {
      entity[prop] = this.#addValues(ref, prop, entity[prop], values, { duplicate, recurse: true, replace: true, merge: true });
      return Utils.asArrayRef(entity[prop]).length > oldCount;
    } else {
      return this.#setProperty(entity, prop, values, { duplicate, recurse: true, replace: true, merge: true });
    }
  }
  ...
  deleteEntity(id_or_entity, { references = false } = {}) {
    ...
    if (n) {
      ...
      n[$proxy]?.revoke();
      n[$proxy] = undefined;
      if (countAllReverse(n[$noderef]) > 0) this.#getNode(n['@id'], n[$noderef]);
      return true;
    }
    return false;
  }
  ...
  getTerm(definitionId) {
    if (!this.#contextIndex) return;
    return this.#contextIndex[definitionId];
  }
  ...
  getProperty(idOrEntity, prop) {
    let node = this.#toNode(idOrEntity);
    if (node) {
      let val = node[prop];
      ...
      if (val != null) {
        ...
        return (Array.isArray(val) || this.config.array) ? new Proxy(vals, new ArrayHandler(this.#getNodeProxy(node), prop)) : vals[0];
      }
      return val;
    }
  }
