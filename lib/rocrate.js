...
class ROCrate {
  ...
  constructor(json = {}, config) {
    ...
    this.config.replace =
      config?.replace ?? config?.['replaceExisting'] ?? false;
    this.config.merge = config?.merge ?? config?.['mergeProperties'] ?? false;
    this.config.duplicate =
      config?.duplicate ?? config?.['allowDuplicates'] ?? false;
    ...
    this.#handlerReverse = {
      get(target, prop) {
        let vals = Utils.asArray(target[prop]).map((v) =>
          that.config.link ? that.getEntity(v['@id']) : v
        );
    ...
    if (!Array.isArray(g) || !g.length)
      g = [defaults.datasetTemplate, defaults.metadataFileDescriptorTemplate];
  ...
  #updateNode(
    node,
    data,
    { replace = this.config.replace, merge = this.config.merge, recurse }
  ) {
    var keys = Object.keys(data).filter(
      (prop) => prop !== '@id' && prop !== '@reverse'
    );
    ...
    if (!node['@type']) {
      this.#setProperty(node, '@type', 'Thing', {});
    }
  ...
  addEntity(data, replace = this.config.replace, recurse) {
    ...
    let r = this.#updateNode(n, data, { replace, recurse });
  ...
  deleteEntity(id_or_entity, { references = false } = {}) {
    ...
    if (n) {
      ...
      n[$proxy]?.revoke();
      if (countAllReverse(n[$noderef]) > 0)
        this.#getNode(n['@id'], n[$noderef]);
      return true;
    }
    return false;
  }
  ...
  updateEntity(data, merge = this.config.merge, recurse) {
    ...
    if (n && n[$size])
      return this.#updateNode(n, data, { replace: true, merge, recurse });
    return false;
  }
  ...
  getProperty(idOrEntity, prop) {
    ...
    if (entity) {
      ...
      if (val != null) {
        let vals = Utils.asArray(val).map((v) =>
          this.config.link && v?.['@id'] ? this.getEntity(v['@id']) || v : v
        );
  ...
  #setProperty(entity, prop, values, { duplicate, replace, recurse, merge }) {
    ...
    if (entity[prop]) {
      ...
      mapValue(
        entity[prop],
        (v) => !newIds.has(v['@id']) && removeReverse(ref, prop, v)
      );
    }
    if (values == null) {
      entity[prop] = values;
      ...
      entity[prop] =
        this.#addValues(ref, prop, [], values, {
          duplicate,
          replace,
          recurse,
          merge,
  ...
  setProperty(idOrEntity, prop, values, duplicate) {
    let entity = this.#toNode(idOrEntity);
    if (!entity)
      throw new Error('Cannot set property of a non-existant entity');
    if (prop === '@reverse') throw new Error('@reverse property is read only');
    if (prop === '@id') {
      if (typeof values === 'string')
        return this.updateEntityId(entity, values);
      else return false;
    }
    return this.#setProperty(entity, prop, values, {
      duplicate,
      recurse: true,
    });
  }
  ...
  deleteProperty(idOrEntity, prop) {
    if (prop === '@id' || prop === '@reverse' || prop === '@type')
      throw new Error(`Property ${prop} is not allowed be deleted`);
    let entity = this.#toNode(idOrEntity);
  ...
  deleteValues(idOrEntity, prop, values) {
    ...
    if (entity && prop in entity) {
      ...
      let r = Utils.asArray(entity[prop]).filter(
        (v) => !vals.some((val) => Utils.isEqualRef(v, val))
      );
      if (r.length) {
        entity[prop] = r;
      } else {
  ...
  addValues(idOrEntity, prop, values, duplicate) {
    ...
    if (prop in entity) {
      entity[prop] = this.#addValues(ref, prop, entity[prop], values, {
        duplicate,
        recurse: true,
        replace: true,
        merge: true,
  ...
  /**
   * Get a citation string in the recommended datacite format .
   * @return {Object} -- COntains citationstring if it was posible to make one or a report if not
   */
  getCitationString() {
    // Find some Creators
    var report = {};
    var creatorStrings = [];
    const root = this.rootDataset;
    for (const prop of ['author', 'creator']) {
      const vals = this.getProperty(root, prop);
      if (vals) {
        console.log(vals);
        for (let creator of vals) {
          if (!creator['@id']) {
            // Assume is a string
            creatorStrings.push(creator);
          } else if (creator.familyName && creator.givenName) {
            creatorStrings.push(`${creator.familyName}, ${creator.givenName}`);
          } else if (creator['name']) {
            creatorStrings.push(`${creator.name}`);
          }
        }
      }
    }
    ...
    if (identifier.match(/https?:/i)) {
      ...
      report.id = 'Crate MUST have an @id that is an HTTP URI';
    }
    var datePublished = root.datePublished;
    if (!datePublished) {
      report.datePublished = 'Root dataset must have a datePublished property';
    }
    var publisher = '';
    if (root.publisher) {
      if (root.publisher[0]['@id']) {
          if (root.publisher[0].name) {
            publisher = root.publisher[0].name;
          } else {
            publisher = publisher[0];
          }
        } else {
          publisher = root.publisher[0];
        }
    } 
    if (!publisher) {
      report.publisher =
        'Root dataset must have a publisher property with a name, or a refenence to same';
    }
    var name = ''
    if (root.name) {
      name = root.name;
    } else {
      report.name = 'Root dataset must have a name';
    }

    if (creatorStrings.length === 0) {
      report.creator =
        'ROOT Dataset must have at least one creator or property with a string value or linking to an entity with a name or a givenName AND familyName.\n';
    }
    var citation;
    if (Object.keys(report).length === 0) {
      citation = `${creatorStrings.join(
        '; '
      )} (${datePublished}) ${name}. ${publisher}. ${root['@type'].join(",")}. ${identifier}`;
      report = {};
    }
    return { report: report, citation: citation };

    // TODO - PUBLISHER BY REFERENCE
  }
  ...
  getIdentifier(name) {
    ...
    const identifier = mapValue(root['identifier'], (v) => {
      ...
      if (
        idEntity &&
        this.hasType(idEntity, 'PropertyValue') &&
        idEntity.name === name
      )
        return idEntity;
    });
  ...
  getTree({
    ...
    if (depth == Infinity && allowCycle)
      throw new Error(
        'Option allowCycle must be set to false is depth is not finite'
      );
  ...
  resolve(items, pathArray, subgraph) {
            ...
            if (!resolvedIds[id]) {
              ...
              if (p.includes) {
                for (let inc of Object.keys(p.includes)) {
                  if (
                    Utils.asArray(potentialItem[inc]).includes(p.includes[inc])
                  ) {
    ...
    if (subgraph) {
      for (const item of resolvedArray) {
        if (!subgraph.find((e) => e['@id'] === item['@id']))
          subgraph.push(item);
      }
    }
  ...
  #getDefinition(contextIndex, term) {
    /*
    {
    ...
    */
    var def = { '@id': this.#resolveTerm(contextIndex, term) };
    let localDef;
    if (def['@id'] && (localDef = this.getEntity(def['@id']))) {
      ...
      if (
        localDef &&
        (this.hasType(localDef, 'rdfs:Class') ||
          this.hasType(localDef, 'rdf:Property'))
      ) {
        def = localDef;
      }
    }
  ...
  resolveTerm(term) {
    if (!this.#contextIndex) return;
    return this.#resolveTerm(this.#contextIndex, term);
  }

  #resolveTerm(contextIndex, term) {
...
function countAllReverse(ref) {
  return Object.values(ref['@reverse']).reduce(
    (count, refs) => count + Utils.asArray(refs).length,
    0
  );
}
