...
const contextCache = {
  [defaults.roCrateContextUrl]: require('./context.json')?.['@context']
};
...
class ROCrate {
  ...
  constructor(json = {}, config) {
    ...
    for (let c of this.__context) {
      if (typeof c === 'string') this.__contextCache[c] = contextCache[c];
    }
  ...
  /** 
   * The Metadata Descriptor, which describes the RO-Crate Metadata Document and links it to the Root Data Entity.
   */
  get descriptor() {
  ...
  /** @deprecated Alias for {@link descriptor} */
  get metadataFileEntity() { return this.descriptor; }

  /** @deprecated Alias for {@link descriptor} */
  get metadataFileDescriptor() { return this.descriptor; }
  ...
  addContext(context) {
    for (let c of Utils.asArrayRef(context)) {
      if (!this.__context.includes(c)) {
        ...
        if (typeof c === 'string' && contextCache[c]) {
          this.__contextCache[c] = contextCache[c];
        }
      }
    }
  }
   ...
   * @param {boolean} [refreshCache] If true, always fetch the content from the remote URL
   */
  async importContext(contextUrl, refreshCache) {
    for (let c of Utils.asArrayRef(contextUrl)) {
      if (typeof c === 'string') {
        const rc = await resolveRemoteContext(c, refreshCache);
        if (rc) {
  ...
  addEntity(data, { replace, recurse } = {}) {
    if (!data || !data['@id'] || typeof data['@id'] !== 'string') return false;
  ...
  async resolveContext(refreshCache = false) {
    ...
    let results = await Promise.allSettled(this.__context.map(async (contextUrl) => {
      if (typeof contextUrl === 'string') {
        ...
        if (!c || refreshCache) {
          c = await resolveRemoteContext(contextUrl, refreshCache);
          if (c) this.__contextCache[contextUrl] = c;
        }
      }
    }));
    return {
 ...
 * @returns 
 */
async function resolveRemoteContext(contextUrl, refreshCache) {
  let c = contextCache[contextUrl];
  if (!c || refreshCache) {
    try {
