...
describe("Incremental checking", async function () {
  ...
  it("should trigger all the right reporting", async function () {
    ...
    assert(hasMessage(validator.result.warnings, "Root Data Entity has appropriate @id. Is: Nothing special", "Nothing special"))
    ...
    var validator = new Validator(crate.toJSON());
    validator.rootDataEntity();
    assert(validator.result.errors[0].clause === "@type: MUST be [Dataset] or an array that contain Dataset");
    ...
    assert(hasMessage(validator.result.errors, "Missing required property: datePublished"))
